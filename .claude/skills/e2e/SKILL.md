...
> **Note on gitignored paths:** `tests//e2e-runs/` is gitignored to keep screenshots and logs out of version control, but they are still fully readable from disk. Always use the `Read` tool with the explicit file path to view screenshots and reports — `.gitignore` only affects git, not filesystem access.
